...
{
	IplImage *diffQueue[MAX_QUEUE] = { NULL };//用于三帧帧差法的循环队列,增加一个空位用来队列判满  
	int front, rear;                //队头，队尾  
	int ncount = 0;                 //记录帧数  
	IplImage *frontImg = NULL;  //前一帧  
	IplImage *midImg = NULL;    //中间帧  
	...
	while (1)
	{
		...
		if (ncount > 0)
		{
			...
			//队列不满则帧继续入队  
				...
				//printf("\n入队\n");   
				rear = (rear + 1) % MAX_QUEUE;
				cvCvtColor(mframe, diffQueue[rear], CV_BGR2GRAY);     //图像入队  
			}
			//队满,即队列中符合要求的视频帧数为 MAX_QUEUE-1，开始对中间帧的每一个ROI进行帧差法  
			else {
				frontImg = diffQueue[(front + 1) % MAX_QUEUE];
				midImg = diffQueue[(front + MAX_QUEUE/2) % MAX_QUEUE];
				rearImg = diffQueue[(front + MAX_QUEUE-1) % MAX_QUEUE];
				cvAbsDiff(midImg, frontImg, Imask1);
				cvAbsDiff(rearImg, midImg, Imask2);
				...
				cvCalcHist(&Imask, hist);
				cvNormalizeHist(hist, 1);
				int max_value = 0;
				cvGetMinMaxHistValue(hist, 0, 0, 0, &max_value);
				if (max_value == 255)
				{
					CvFont font;
					cvInitFont(&font, CV_FONT_HERSHEY_SIMPLEX, 1.0f, 1.0f, 0, 2, CV_AA);
					cvPutText(mframe, "Tampering", cvPoint(5, 30), &font, cvScalar(0,255,0));
				}
				cvShowImage("diff Frame", mframe);
				cvWaitKey(1);
				front = (front + 1) % MAX_QUEUE;        //前一帧出队  
			}
		}
